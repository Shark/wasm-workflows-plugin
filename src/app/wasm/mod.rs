...
pub async fn run(
    ...
    let mut module: Option<Vec<u8>> = cache.get(oci_image).map_err(|err| {
        WasmError::EnvironmentSetup(anyhow!(err).context("Checking Wasm module cache failed"))
    })?;
    if module.is_none() {
        let pulled_mod = pull(oci_image, insecure_oci_registries)
            .await
            .map_err(|err| {
        ...
        let _ = cache.put(oci_image, &precompiled_mod).map_err(|err| {
            WasmError::Retrieve(anyhow!(err).context("Storing Wasm module in cache failed"))
        })?;
        module = Some(precompiled_mod);
    }

    let (mut store, wf) = setup(engine, module.unwrap()).await.map_err(|err| {
        WasmError::EnvironmentSetup(anyhow!(err).context("Wasm module invocation failed"))
    })?;
    ...
    let outputs: Option<Outputs> = match node.parameters.len() {
        ...
        _ => {
            let out_params = node
                ...
                .map_err(|err| {
                    WasmError::OutputProcessing(
                        anyhow!(err).context("Error processing Wasm result node"),
                    )
                })?;
...
async fn setup(
    ...
    let (wf, _instance) = workflow::Workflow::instantiate(
        &mut store,
        &module,
        &mut linker,
