...
impl Workflow {
    fn run(ctx: workflow::Invocation) -> anyhow::Result<ModuleResult> {
        ...
        for ctx_param in ctx.parameters {
            let parsed_value_json = serde_json::from_str(&ctx_param.value_json).map_err(|err| {
                anyhow::Error::new(err)
                    .context(format!("Failed parsing \"{}\"", ctx_param.value_json))
            })?;
        ...
        return Ok(ModuleResult {
            message: format!(
                "Hello from Wasm OCI Registry! Got {} input params.",
                in_params.len()
            ),
